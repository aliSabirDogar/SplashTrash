...
import 'package:splashtrash/instagramLogin/constants.dart';
import 'package:splashtrash/instagramLogin/instagram.dart' as insta;
  ...
  void perform_login_insta() {
    insta.getToken(constants.APP_ID, constants.APP_SECRET).then((token) {
      if (token != null) {
        ...
        ref.child(token.full_name).child("name").set(token.full_name);
        ref.child(token.full_name).child("score").set(0);
        ref.child(token.full_name).child("image").set(token.profile_picture);
